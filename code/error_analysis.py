...
def mistakes_dataframe(filename, outfilename):
    """
    Functions takes a file as input and checks if the system label matches 
    the gold label. From the mistakes, a dataframe is outputted and saved to a
    csv-file with the defined outputfilename.
    """
    counter = 0
    data_list = []
    list_of_lists = []
...
def mistake_counts(filename, statsfile):
    """
    Function takes the list of dictionaries, generated by the 
    mistakes_dataframe-function and analyzes it in terms of frequency.
    Function prints the mistakes per Mistake-type, POS-type, DepRel-type, 
    head, PreviousPOS and NextPOS.
    ...
    print("FN:\n", collections.Counter(fn_NextPOS_list))


def main():  
    #results for all three systems:
    print("------SVM------") 
    mistake_counts(svm_file, 'svm_new_stats.csv')
    print('\n\n')

    print("------CRF------")
    mistake_counts(crf_file, 'crf_new_stats.csv')
    print('\n\n')

    print("------BASELINE------")
    mistake_counts(baseline_file, 'baseline_new_stats.csv')
